...
$checkSql = "SELECT * FROM sabh WHERE party = ? AND governor = ? AND vicegovernor = ? AND execsecretary = ? AND assoc_secretary = ? AND treasurer = ? AND auditor = ? AND press_relation_officer = ? AND business_manager1 = ? AND business_manager2 = ? AND logistic_head = ? AND socio_cultural_coordinator = ? AND ceca_coordinator = ? AND sports_coordinator = ? AND environmental_coordinator = ? AND religious_coordinator = ?";
...
if ($result->num_rows > 0) {
    ...
    $sql = "INSERT INTO sabhvotes (department, party, governor, vice_governor, exec_secretary, assoc_secretary, treasurer, auditor, press_relation_officer, business_manager1, business_manager2, logistic_head, socio_cultural_coordinator, ceca_coordinator, sports_coordinator, environmental_coordinator, religious_coordinator) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
