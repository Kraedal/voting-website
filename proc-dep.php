...
$checkSql = "SELECT * FROM seaitevotes WHERE party = ? AND governor = ? AND vice_governor = ? AND exec_secretary = ? AND assoc_secretary = ? AND treasurer = ? AND auditor = ? AND press_relation_officer = ? AND business_manager1 = ? AND business_manager2 = ? AND logistic_head = ? AND socio_cultural_coordinator = ? AND ceca_coordinator = ? AND sports_coordinator = ? AND environmental_coordinator = ? AND religious_coordinator = ?";
...
if ($result->num_rows > 0) {
    ...
    $businessManager1 = isset($_POST["Busman1"]) && is_array($_POST["Busman1"]) ? implode(", ", $_POST["Busman1"]) : '';
    $businessManager2 = isset($_POST["Busman2"]) && is_array($_POST["Busman2"]) ? implode(", ", $_POST["Busman2"]) : '';     
    $sql = "INSERT INTO seaite (department, party, governor, vice_governor, exec_secretary, assoc_secretary, treasurer, auditor, press_relation_officer, business_manager1, business_manager2, logistic_head, socio_cultural_coordinator, ceca_coordinator, sports_coordinator, environmental_coordinator, religious_coordinator) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
