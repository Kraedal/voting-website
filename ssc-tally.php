...
$checkSql = "SELECT * FROM ssc WHERE party = ? AND president = ? AND vice_president = ? AND exec_secretary = ? AND assoc_secretary = ? AND treasurer = ? AND auditor = ? AND press_relation_officer = ? AND business_manager1 = ? AND business_manager2 = ? AND logistic_head = ? AND socio_cultural_coordinator = ? AND ceca_coordinator = ? AND sports_coordinator = ? AND environmental_coordinator = ? AND religious_coordinator = ?";
...
if (mysqli_num_rows($result) > 0) {
    ...
    $sql = "INSERT INTO sscvotes (party, president, vice_president, exec_secretary, assoc_secretary, treasurer, auditor, press_relation_officer, business_manager1, business_manager2, logistic_head, socio_cultural_coordinator, ceca_coordinator, sports_coordinator, environmental_coordinator, religious_coordinator)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    $stmt->bind_param("ssssssssssssssss", $party, $president, $vicePresident, $execSecretary, $assocSecretary, $treasurer, $auditor, $pressRelationOfficer, $businessManager1, $businessManager2, $logisticHead, $socioCulturalCoordinator, $cecaCoordinator, $sportsCoordinator, $environmentalCoordinator, $religiousCoordinator);
    $stmt->execute();
