...
$checkSql = "SELECT * FROM seaite WHERE party = ? AND governor = ? AND vice_governor = ? AND exec_secretary = ? AND assoc_secretary = ? AND treasurer = ? AND auditor = ? AND press_relation_officer = ? AND business_manager1 = ? AND business_manager2 = ? AND logistic_head = ? AND socio_cultural_coordinator = ? AND ceca_coordinator = ? AND sports_coordinator = ? AND environmental_coordinator = ? AND religious_coordinator = ?";
$stmt = mysqli_stmt_init($conn);
if (!mysqli_stmt_prepare($stmt, $checkSql)) {
    die(mysqli_error($conn));
}

mysqli_stmt_bind_param($stmt, "ssssssssssssssss", $party, $governor, $viceGovernor, $execSecretary, $assocSecretary, $treasurer, $auditor, $pro, $busMan1, $busMan2, $logisticHead, $socioCulturalCoordinator, $cecaCoordinator, $sportsCoordinator, $environmentalCoordinator, $religiousCoordinator);
...
if (mysqli_num_rows($result) > 0) {
    ...
    $sql = "INSERT INTO seaitevotes (party, governor, vice_governor, exec_secretary, assoc_secretary, treasurer, auditor, press_relation_officer, business_manager1, business_manager2, logistic_head, socio_cultural_coordinator, ceca_coordinator, sports_coordinator, environmental_coordinator, religious_coordinator)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
